...
std::vector<float> OpenGLSpriteRenderer::getBufferData() {
    ...
    for (auto it = 0; it < sprites.size(); it++) {
        ...
        //Top left
        vertices[it * 12 + 0] = sprite.position.x;
        vertices[it * 12 + 1] = sprite.position.y;

        //Top right
        vertices[it * 12 + 2] = sprite.position.x + sprite.size.x;
        vertices[it * 12 + 3] = sprite.position.y;

        //Bottom right
        vertices[it * 12 + 4] = sprite.position.x + sprite.size.x;
        vertices[it * 12 + 5] = sprite.position.y + sprite.size.y;

        //Bottom right
        vertices[it * 12 + 6] = sprite.position.x + sprite.size.x;
        vertices[it * 12 + 7] = sprite.position.y + sprite.size.y;

        //Bottom left
        vertices[it * 12 + 8] = sprite.position.x;
        vertices[it * 12 + 9] = sprite.position.y + sprite.size.y;

        //Top left
        vertices[it * 12 + 10] = sprite.position.x;
        vertices[it * 12 + 11] = sprite.position.y;
    }
